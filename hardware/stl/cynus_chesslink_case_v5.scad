// CynusLink ESP32-S3 gateway enclosure V5
...
// V5: V4 geometry with snap pockets raised to align with the lid bumps.
...
module base_shell(){
    difference(){
        ...
        // V5: raised to match the lid bumps. The lid bumps sit about
        // 1.15..2.00 mm below the base top edge when closed.
        // Pocket range is about 0.95..2.10 mm below the top edge,
        // giving a little vertical clearance for FDM tolerances.
        snap_pocket_z = base_h - 2.10;
        snap_pocket_h = 1.15;
        for(x=[outer_l*0.31, outer_l*0.69]){
            translate([x-2.2, wall-0.05, snap_pocket_z])
                cube([4.4,1.25,snap_pocket_h]);
            translate([x-2.2, outer_w-wall-1.20, snap_pocket_z])
                cube([4.4,1.25,snap_pocket_h]);
        }
    }
