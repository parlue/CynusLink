...
class ServerCB : public NimBLEServerCallbacks {
    void onConnect(
        ...
        // Both BLE sides are now connected.
        // Engine-off was already sent twice before ChessLink advertising
        // was enabled. Now synchronize the initial board only.
        if (cynusReady) {
            boardSynced = false;
        ...
        if (c == '\n') {
            ...
            if (line.startsWith("fen:")) {
                ...
                if (fen2board(f)) {
                    ...
                    // --------------------------------------------------------
                    //
                    // During a running session, a scanned physical start
                    // position means "start a new game". This must work from
                    // every move-cycle state, including an abandoned game
                    // that was still waiting for an engine or robot move.
                    String placement = bufferedFen;
                    int placementSpace = placement.indexOf(' ');

                    if (placementSpace >= 0) {
                        placement = placement.substring(0, placementSpace);
                    }
                    ...
                    if (
                        ...
                        Serial.println(
                            "[NEW GAME] initial position detected"
                        );

                        // The freshly scanned start position becomes the
                        // authoritative board state for both sides.
                        fenNow = bufferedFen;
                        boardSynced = true;

                        // Cancel every pending action from the previous game.
                        publishNextFenToChessLink = false;
                        cynusWaitingForMove = false;
                        memset(led, 0, sizeof(led));
                        setMoveCycle(WAIT_HUMAN_MOVE);
                        ...
                        Serial.println(
                            "[NEW GAME] previous move cycle cleared"
                        );

                        // Publish immediately. New-game recognition must not
                        // depend on the previous game's auto-report setting
                        // or on a pending ChessLink S request.
                        if (clNotify) {
                            sendStatus();

                            Serial.println(
                                "[NEW GAME] initial board sent to ChessLink"
                            );
                        } else {
                            Serial.println(
                    ...
                    // Initial synchronization after both sides connected:
                    // accept the first valid FEN and enter RUNNING.
                    if (
                        state == SYNC_BOARD &&
                        cynusReady &&
                        clConnected
...
class CClientCB : public NimBLEClientCallbacks {
    void onDisconnect(
        ...
        // No robot = ChessLink side must not stay available.
        if (clConnected && clServer) {
            Serial.println(
                "[CHESS] robot lost - disconnecting ChessLink client"
            );

            clServer->disconnect(
                clServer->getPeerInfo(0).getConnHandle()
            );
        }
...
void loop() {
    ...
    // Only after both engine-off sends do we expose the ChessLink side.
    if (
        ...
        Serial.println(
            "[CYNUS] engine-off sequence complete; enabling ChessLink advertising"
        );
